...
class ContigousTransition(nn.Module):
    ...
    def add_noise_to_ref(self, x, time_step, batch):
        if self.num_classes is not None:  # categorical values using continuous noise
            x = F.one_hot(x, self.num_classes).float()
        x = x / self.scaling
        # Xt = a.sqrt() * X0 + (1-a).sqrt() * eps
    ...
    def get_prev_from_recon_pocket(self, x_t, x_recon,ref, t, batch, n_samples,ref_mask,condition_clash,pocket_x,clash_rate, device):
        ...
        mu = mu - mu * ref_mask[:, None]  + ref * ref_mask[:, None]

        if condition_clash:
            pocket_x_t = pocket_x / self.scaling
            noise = torch.zeros_like(pocket_x_t).to(pocket_x_t)
            noise.normal_()

            with torch.enable_grad():
                x_in = mu.detach().requires_grad_(True)
                sigma = 2.
                e = torch.exp(
                    -torch.sum((pocket_x_t.view(1, -1, 3) - x_in.view(-1, 1, 3) )** 2, dim=2) / float(sigma))  # (m, n)
                distances_loss =  -sigma * torch.log(1e-3 + e.sum(dim=1))
                selected = scatter_mean(distances_loss, batch).sum()
                if selected == None:
                    grads = [torch.zeros_like(x_in)]
                else:
                    grads = torch.autograd.grad(selected, x_in)

                mu = mu + grads[0] * clash_rate
        else:
            grads = [0]
...
class GeneralCategoricalTransition(nn.Module):
    ...
    def q_v_posterior_with_ref(self, log_v0, log_vt, t, batch, v0_prob, ref_type, ref_mask):
        # q(vt-1 | vt, v0) = q(vt | vt-1, x0) * q(vt-1 | x0) / q(vt | x0)
        t_minus_1 = t - 1
        t_minus_1 = torch.where(t_minus_1 < 0, torch.zeros_like(t_minus_1),
                                t_minus_1)  # Remove negative values, will not be used anyway for final decoder
